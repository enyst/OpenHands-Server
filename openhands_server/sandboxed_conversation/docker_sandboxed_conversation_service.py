...
from openhands_server.local_conversation.local_conversation_models import ConversationStatus
...
from openhands_server.sandboxed_conversation.sandboxed_database_models import StoredSandboxedConversationInfo, StoredConversationMetricsSnapshot
        ...
        try:
            ...
            async with httpx.AsyncClient() as client:
                ...
                if response.status_code == 200:
                    ...
                    events = [EventBase.model_validate(event) for event in page_data.get("items", [])]
                    return EventPage(
                        items=events,
                        next_page_id=page_data.get("next_page_id")
                    )
                else:
                    response.raise_for_status()
        except (httpx.RequestError, httpx.HTTPStatusError):
...
class DockerSandboxedConversationService(SandboxedConversationService):
    ...
    def _sandbox_status_to_conversation_status(self, sandbox_status: SandboxStatus) -> ConversationStatus:
    ...
    async def _stored_to_sandboxed_info(self, stored: StoredSandboxedConversationInfo, metrics: Optional[StoredConversationMetricsSnapshot] = None) -> SandboxedConversationInfo:
        ...
        if stored.sandbox_id:
            ...
            if sandbox_info:
                status = self._sandbox_status_to_conversation_status(sandbox_info.status)
    ...
    async def search_sandboxed_conversations(self, user_id: UUID | None = None, page_id: str | None = None, limit: int = 100) -> SandboxedConversationPage:
        ...
        async with AsyncSessionLocal() as session:
            # Build query
            query = select(StoredSandboxedConversationInfo).order_by(StoredSandboxedConversationInfo.created_at.desc())
            ...
            for stored in stored_conversations:
                ...
                metrics_query = select(StoredConversationMetricsSnapshot).where(
                    StoredConversationMetricsSnapshot.conversation_id == stored.conversation_id
                ).order_by(StoredConversationMetricsSnapshot.id.desc()).limit(1)
                
                metrics_result = await session.execute(metrics_query)
                metrics = metrics_result.scalar_one_or_none()
                
                conversation_info = await self._stored_to_sandboxed_info(stored, metrics)
    ...
    async def batch_get_sandboxed_conversations(self, user_id: UUID, conversation_ids: list[UUID]) -> list[SandboxedConversationInfo | None]:
        ...
        async with AsyncSessionLocal() as session:
            ...
            result = await session.execute(query)
            stored_conversations = {conv.conversation_id: conv for conv in result.scalars().all()}
            
            # Get all metrics in one query
            metrics_query = select(StoredConversationMetricsSnapshot).where(
                StoredConversationMetricsSnapshot.conversation_id.in_(conversation_ids)
            ).order_by(StoredConversationMetricsSnapshot.conversation_id, StoredConversationMetricsSnapshot.id.desc())
    ...
    async def get_event_context(self, id: UUID) -> ReadOnlyEventContext | None:
        ...
        conversation = await self.get_sandboxed_conversation(None, id)  # user_id not needed for this lookup
